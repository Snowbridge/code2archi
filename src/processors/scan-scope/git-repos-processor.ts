...
export class GitReposProcessor implements IProcessor<ScanScopeInput, ScanScopeOutput> {
  ...
  private static readonly BUILD_SYSTEM_FILES: Record<
    string,
    (typeof GitReposProcessor.BUILD_SYSTEM_ORDER)[number]
  > = {
    "pom.xml": "maven",
    "build.gradle": "gradle",
  ...
  process(input: ScanScopeInput): ScanScopeOutput {
    return this.buildRepositoriesFromSourceDirs(input);
  }

  private buildRepositoriesFromSourceDirs(sourceDirs: readonly string[]): Repository[] {
    const repoRoots = GitWorkingCopy.findRepoRootsInSourceDirs(sourceDirs);
    return repoRoots.map((repoRoot) => this.buildRepositoryFromRoot(sourceDirs, repoRoot));
  }

  private buildRepositoryFromRoot(
    ...
    return {
      ...
      namespace: this.computeRepositoryNamespace(sourceDirs, localPath),
      localPath,
      url,
      buildSystems: this.detectBuildSystems(localPath),
    };
  }

  private computeRepositoryNamespace(
    sourceDirs: readonly string[],
    localPath: string,
  ): string {
    const resolvedLocalPath = path.resolve(localPath);
    const commonPrefix = this.findCommonPathPrefix(sourceDirs);
  ...
  private findCommonPathPrefix(paths: readonly string[]): string {
    if (paths.length === 0) {
      return "";
    }
  ...
  private detectBuildSystems(repoRoot: string): string[] {
    ...
    for (const entry of entries) {
      const buildSystem = GitReposProcessor.BUILD_SYSTEM_FILES[entry];
      if (buildSystem && existsSync(path.join(resolvedRoot, entry))) {
        found.add(buildSystem);
      }
    }

    return GitReposProcessor.BUILD_SYSTEM_ORDER.filter((system) => found.has(system));
  }
}
