...
import {
  ...
  nodejsInferredContractAssignmentId,
  nodejsInferredContractAssignmentLogicalId,
...
const GENERATOR_COORDINATE =
  "generate.elements.application.rest.nodejs:nodejs-controllers-inferred-api-contracts";

const REQUIRED_PROFILES: readonly ArchiProfile[] = [InferredApiContractProfile.create()];

export class NodejsControllersInferredApiContractsProcessor extends AbstractProcessor<
  GenerateProcessorInput,
  ArchiCreateIntents
> {
  readonly id: ProcessorId = {
    groupId: "generate.elements.application.rest.nodejs",
    artifactId: "nodejs-controllers-inferred-api-contracts",
  };
  ...
  readonly description =
    "Maps NodejsRestController endpoints and dtoTypes to inferred ApplicationInterfaces.";
  ...
  protected doProcess(input: GenerateProcessorInput): ArchiCreateIntents {
    ...
    for (const controller of controllers) {
      ...
      if (!isEligibleForNodejsInferredRestContract(controller.endpoints, controller.dtoTypes)) {
        continue;
      }
      ...
      const contractId = nodejsInferredRestContractId(controller.qualifiedSymbol);
      ...
      if (
        ...
        let elementBuilder = ApplicationInterface.withId(contractId)
          .name(
            decorateElementName("nodejs-inferred-rest-contract", controller.name, {}, input.options),
          )
          .documentation(
            buildNodejsInferredContractDocumentation(controller.endpoints, controller.dtoTypes),
          )
        ...
        for (const property of standardGenerateElementProperties({
          logicalId: nodejsInferredRestContractLogicalId(controller.qualifiedSymbol),
      ...
      const relationId = nodejsInferredContractAssignmentId(contractId, controller.id);
      ...
      for (const property of standardGenerateElementProperties({
        logicalId: nodejsInferredContractAssignmentLogicalId(
          controller.qualifiedSymbol,
          controller.id,
        ),
        generatorCoordinate: GENERATOR_COORDINATE,
        slot: "nodejs-inferred-contract-assigned-to-rest-controller",
        confidence: "inferred",
      })) {
