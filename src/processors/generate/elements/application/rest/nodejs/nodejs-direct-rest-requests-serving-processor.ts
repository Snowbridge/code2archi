...
import type { NodejsDirectRestRequestsServingMatchRecord } from "../../../../../../discovery-model/links/nodejs-direct-rest-requests-serving-match.js";
import {
  AbstractProcessor,
...
const GENERATOR_COORDINATE =
  "generate.elements.application.rest.nodejs:nodejs-direct-rest-requests-serving";
...
> {
  ...
  readonly description =
    "Maps NodejsDirectRestRequestsServingMatch links to ServingRelationships between application module components.";
  ...
  protected doProcess(input: GenerateProcessorInput): ArchiCreateIntents {
    ...
    const matches = input.discovery
      .listLinks("NodejsDirectRestRequestsServingMatch")
      .map((record) => record as unknown as NodejsDirectRestRequestsServingMatchRecord)
      .map(
    ...
    for (const match of winners) {
      ...
      for (const property of standardGenerateElementProperties({
        ...
        slot: "nodejs-direct-rest-requests-serving",
