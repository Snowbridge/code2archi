...
> {
  ...
  protected doProcess(input: GenerateProcessorInput): ArchiCreateIntents {
    ...
    const profiles = [RestClientProfile.create()].filter(
    ...
    const dependencies = [...input.discovery.listEntities("ApplicationModuleDependency")].map(
      (record) => record as unknown as ApplicationModuleDependencyRecord,
    );
