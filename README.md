...
Each stage is a separate CLI command. Stages communicate only through **artifacts on disk** (code-inventory JSON, then `.archimate`), so the workflow is reproducible in a terminal or CI without ad-hoc scripts.
...
This writes a **code-inventory** directory, by default `code2archi-scan-<timestamp>` in the current working directory.
...
Or point explicitly at a code-inventory folder:
...
c2a scan --with-only scan.scope.git-repositories /path
```
...
- Application components per module
- REST API contract interfaces
...
| **Element slot coverage** | All documented `generate.elements` slots materialised in `.archimate` | 14/14 documented element slots implemented |
