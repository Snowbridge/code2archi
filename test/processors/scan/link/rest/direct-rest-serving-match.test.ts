...
} from "../../../../../src/processors/scan/link/rest/direct-rest-serving-match.js";
...
function controller(
  ...
  return {
    ...
    scannerExtractor: "scan.source:test",
    scannerSchema: "0.0.0",
...
describe("direct-rest-serving-match", () => {
  ...
  it("creates confirmed INTERFACE match with score 1", () => {
    ...
    assert.equal(matches[0]?.confidenceScore, 1);
  ...
  it("creates inferred DTO match with capped score below 1", () => {
    ...
    assert.ok(dtoMatch);
    assert.equal(dtoMatch.confidence, "inferred");
    assert.ok(dtoMatch.confidenceScore > 0.55);
    assert.ok(dtoMatch.confidenceScore <= 0.85);
    assert.ok(dtoMatch.confidenceScore < 1);
  });
  ...
  it("creates inferred ENDPOINT match below DTO score for same overlap ratio", () => {
    ...
    assert.ok(endpointMatch.confidenceScore < dtoMatch.confidenceScore);
    assert.ok(endpointMatch.confidenceScore <= 0.5);
  });
