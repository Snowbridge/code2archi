...
import { RestAppComponentClientsProcessor } from "../../../../../../src/processors/generate/elements/application/rest/app-component-clients-processor.js";
...
describe("RestAppComponentClientsProcessor", () => {
  it("exposes generate.elements.application.rest coordinates with ON_DEMAND policy", () => {
    const processor = new RestAppComponentClientsProcessor();
  ...
  it("creates service and realization for clients in non-library modules only", () => {
    ...
    assert.equal(
      ...
      "generate.elements.application.rest:app-component-clients",
    );

    assert.equal(output.relations?.length, 1);
    assert.equal(output.relations?.[0]?.relationType, "RealizationRelationship");
    assert.equal(output.relations?.[0]?.id, appModuleRealizesRestClientId(appModule.id, appClient.id));
  });

  it("skips everything when all clients live in library modules", () => {
    const { repository, libModule, appModule, dependency } = demoFixture();
    const libClient = clientIn(libModule.id, "LibUserClient");
    const discovery = discoverySnapshot({
      repositories: [repository],
