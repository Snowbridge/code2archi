...
describe("ControllersInferredApiContractsProcessor", () => {
  ...
  it("deduplicates interface and creates two assignments for shared fqcn across modules", () => {
    ...
    store.addCreateIntents(
      "generate.elements",
      { groupId: "generate.elements.application.rest", artifactId: "controllers-inferred-api-contracts" },
      output,
    );
  ...
  it("skips inferred contract when controller has implementedInterfaceFqcn", () => {
    ...
    const declaredOutput = new ControllersDeclaredApiContractsProcessor().process({
      discovery,
      archi,
      options: defaultGenerateProcessorOptions,
    });
    const inferredOutput = new ControllersInferredApiContractsProcessor().process({
      discovery,
      archi,
      options: defaultGenerateProcessorOptions,
    });
